...
private:
	...
	unsigned long m_widthO = 1, m_heightO = 1;

public:
	const unsigned long*mwd = &m_widthO,*mhg=&m_heightO;
	...
	*/
	...
	friend MatrixF operator*(const MatrixF& m1, const MatrixF& m2);
	friend MatrixF operator*(const float& m1, const MatrixF& m2);
	...
	{
		if (this->m_height != m1.m_height || this->m_width != m1.m_width)
		{
			this->m_heightO = m1.m_height;
			this->m_widthO = m1.m_width;
			this->vec.resize(m1.m_height);
			for (long i = 0; i < m1.m_height; ++i)
			{
				this->vec[i].resize(m1.m_width);
			}
		}
...
MatrixF operator*(const float& m1, const MatrixF& m2)
{
	float ans1 = 0;
	MatrixF ans(m2.m_height, m2.m_width);
	for (unsigned long i = 0; i < m2.m_height; ++i)
	{
		for (unsigned long j = 0; j < m2.m_width; ++j)
		{
			ans.vec[i][j] = m2.vec[i] [j] * m1;
		}
	}
	return ans;
}
...
public:
	...
	friend MatrixF operator*(const float& m1, const VectorF& v1);
};
...
MatrixF operator*(const float& m1, const VectorF& v1)
{
	return m1 * v1.mx;
}
